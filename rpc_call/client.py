...
class EventReport:
    ...
    def report_rule_start_event(uuid):
        try:
            MyLog.key('上报规则开始事件 uuid=%s'%(uuid))
            with ClusterRpcProxy(url) as rpc:
                resp = rpc.GW.raiseEvent('ruleStart', uuid=uuid)
        except Exception as e:
            MyLog.error('report_rule_start_event has except: ' + str(e))
            return None
    ...
    def report_rule_end_event(uuid):
        try:
            MyLog.key('上报规则结束事件 uuid=%s'%(uuid))
            with ClusterRpcProxy(url) as rpc:
                resp = rpc.GW.raiseEvent('ruleEnd', uuid=uuid)
        except Exception as e:
            MyLog.error('report_rule_end_event has except: ' + str(e))
            return None
    ...
    def report_rule_command_status_event(uuid, dev_id, command, result_code):
        try:
            MyLog.key('上报规则指令执行结果 uuid=%s, dev_id=%s, command=%s, resultCode=%d'%(uuid, dev_id, command, result_code))
            with ClusterRpcProxy(url) as rpc:
                resp = rpc.GW.raiseEvent('ruleCommandStatus', uuid=uuid, devId=dev_id, command=command, resultCode=result_code)
        except Exception as e:
            MyLog.error('report_rule_command_status_event has except: ' + str(e))
            return None
    ...
    def report_rule_command_cover_event(dev_id, command, be_cover_rule_uuid, cover_rule_uuid):
        try:
            MyLog.key('上报指令被高优先级抢占事件 dev_id=%s, command=%s, be_cover_rule_uuid=%s, cover_rule_uuid=%s'\
            ...
            MyLog.error('report_rule_command_cover_event has except: ' + str(e))
            return None
    ...
    def report_linkage_custom_event(eventId, srcList):
        try:
            MyLog.key('上报用户自定义事件 eventid=%s'%(eventId))
            with ClusterRpcProxy(url) as rpc:
                resp = rpc.GW.raiseEvent('customEvent', eventId=eventId, srcList=srcList)
        except Exception as e:
            MyLog.error('report_linkage_custom_event has except: ' + str(e))
            return None
    ...
    def report_default_command_status_event(dev_id, command, result_code):
        try:
            MyLog.key('上报指令执行默认参数结果事件 dev_id=%s, command=%s, result_code=%d'%(dev_id, command, result_code))
            with ClusterRpcProxy(url) as rpc:
                resp = rpc.GW.raiseEvent('ruleDefaultCommandStatus', devId=dev_id, command=command, resultCode=result_code)
        except Exception as e:
            MyLog.error('report_default_command_status_event has except: ' + str(e))
            return None
    ...
    def report_rule_command_ignore_event(dev_id, command, uuid, higher_priority_uuid):
        try:
            MyLog.key('上报指令忽略执行事件 dev_id=%s, command=%s, uuid=%s, higher_priority_uuid=%s'%(dev_id, command, uuid, higher_priority_uuid))
            with ClusterRpcProxy(url) as rpc:
                resp = rpc.GW.raiseEvent('ruleCommandIgnore', devId=dev_id, command=command, uuid=uuid, higherPriorityUuid=higher_priority_uuid)
        except Exception as e:
            MyLog.error('report_rule_command_ignore_event has except: ' + str(e))
            return None
    ...
    def get_attributes(dev_id, attr_name):
        try:
            with ClusterRpcProxy(url) as rpc:
                ...
                if dev_svr_name:
                    MyLog.key('RPC调用设备(%s)获取属性值(%s)'%(dev_id, attr_name))
            ...
            MyLog.error('get_attributes has except: ' + str(e))
            return None
    ...
    def call_service(dev_id, service_name, params=None)->int:
        try:
            with ClusterRpcProxy(url) as rpc:
                MyLog.info('查询服务名称')
                dev_svr_name = rpc.mng_srv.get_srv_name_from_sn(dev_id)
                MyLog.key('设备(%s)的服务名为%s'%(dev_id, dev_svr_name))
                if dev_svr_name:
                    if params:
                        MyLog.key('RPC调用设备(%s)服务(%s),参数:%s'%(dev_id, service_name, json.dumps(params)))
                        function_name = 'rpc.' + dev_svr_name + '.ioctl'
                        return eval(function_name)(dev_id, service_name, params)
                    else:
                        MyLog.key('RPC调用设备(%s)服务(%s),默认参数'%(dev_id, service_name))
