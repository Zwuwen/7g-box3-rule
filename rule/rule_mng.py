...
class RuleMng:
            ...
            for uuid in uuid_list:
                ...
                for dev_command in dev_command_list:
                    ...
                    for command in dev_command['command_list']:
                        command_info = CommandInfo(uuid, command['service'], command['param'], ts, ts + command['time'],
                                                   priority, 'timer')
                        command_info_list.append(command_info)
                    if command_info_list:
                        # 如果已经添加到了g_running_rule_endtime_list里，说明联动事件已经添加到命令队列里了
                        if not find_running_rule_by_uuid(uuid):
                            DevCommandQueueMng.add_timer_command(dev_command['product_id'], dev_command['dev_id'],
                                                                 command_info_list)
    ...
    def get_rule_timestamp(cls, uuid) -> (float, float):
        ...
        for t in time_list:
            if compare_time(t['start_time'], now_time) and compare_time(now_time, t['end_time']):
                ...
                end_datetime = datetime(now_datetime.year, now_datetime.month, now_datetime.day, end_time.hour,
                                        end_time.minute, end_time.second)
                start_datetime = datetime(now_datetime.year, now_datetime.month, now_datetime.day, start_time.hour,
                ...
                for date in date_list:
                    if now_datetime.year == date['end_date'].year and now_datetime.month == date[
                        'end_date'].month and now_datetime.day == date['end_date'].day:
                ...
                if today_is_end_day:
                    ...
                    end_datetime = datetime(next_day.year, next_day.month, next_day.day, end_time.hour, end_time.minute,
                                            end_time.second)
                start_datetime = datetime(now_datetime.year, now_datetime.month, now_datetime.day, start_time.hour,
                                          start_time.minute, start_time.second)
                is_find_time = True
    ...
    def add_rule(cls, rules) -> int:
        try:
            ...
            for rule_dict in rules:
                ...
                # 将规则插入数据库
                SqliteInterface.add_rule(rule_dict['uuid'], rule_dict['enable'], rule_dict['type'],
                                         rule_dict['priority'], rule_dict['date'], rule_dict['time'],
                ...
                if find_running_rule_by_uuid(uuid):
                    for dev_commands_dict in dev_command_list:
                        ...
                        for command_dict in service_list:
                            MyLog.logger.debug(f'dev_id:{dev_id},cmd:{command_dict["service"]},uuid:{uuid}')
                            EventReport.report_rule_command_ignore_event(dev_id, command_dict['service'], uuid, uuid)
                    continue
                ...
                if allow_exe and (dev_command_list or event_list):
                    ...
                    for dev_command in dev_command_list:
                        ...
                        for command in dev_command['command_list']:
                            ...
                            command_info = CommandInfo(uuid, command['service'], command['param'], current_ts,
                                                       current_ts + command['time'], priority, 'linkage')
                            command_info_list.append(command_info)
                        if command_info_list:
                            DevCommandQueueMng.add_linkage_command(dev_command['product_id'], dev_command['dev_id'],
                                                                   command_info_list)
    ...
    def outside_linkage(cls, services) -> int:
        try:
            ...
            for service_dict in services:
                ...
                if type(service_dict['priority']) != int or service_dict['priority'] < 1 or service_dict[
                    'priority'] > 99:
                    ...
                    for dev_command in dev_command_list:
                        ...
                        for command in dev_command['command_list']:
                            if continue_time < command['time']:
                                continue_time = command['time']
                            command_info = CommandInfo(uuid, command['service'], command['param'], current_ts,
                                                       current_ts + command['time'], service_dict['priority'],
                                                       'linkage')
            ...
            for service_dict in services:
                ...
                if dev_command_list:
                    ...
                    for dev_command in dev_command_list:
                        command_info_list = []
                        for command in dev_command['command_list']:
                            command_info = CommandInfo('manual', command['service'], command['param'], current_ts,
                                                       current_ts + command['time'], service_dict['priority'], 'manual')
                            command_info_list.append(command_info)

                        if command_info_list:
                            DevCommandQueueMng.add_manual_command(dev_command['product_id'], dev_command['dev_id'],
                                                                  command_info_list)
                if os.path.exists(py_path):
                    os.remove(py_path)
                if os.path.exists(pyc_path):
    ...
    def stop_manual_control(cls, services) -> int:
        try:
            ...
            dev_id_list = []
            for service_dict in services:
                DevCommandQueueMng.clear_manual_command(service_dict['productId'], service_dict['devId'],
        ...
        try:
            for key in items:
                if items[key] == 'DATE':
                    ...
                    datetime.strptime('1989-06-04 ' + key, '%Y-%m-%d %H:%M:%S')
        except ValueError:
            ret = False
    ...
    def get_closest_timestamp(cls) -> float:
        ...
        if closest_time == None:
            ...
            next_day = datetime.now() + timedelta(days=1)
            closest_datetime = datetime(next_day.year, next_day.month, next_day.day, smallest_time.hour,
                                        smallest_time.minute, smallest_time.second, smallest_time.microsecond)
        else:
            closest_datetime = datetime(now_datetime.year, now_datetime.month, now_datetime.day, closest_time.hour,
                                        closest_time.minute, closest_time.second, smallest_time.microsecond)# here seems to be a bug
    ...
    def __check_rule_param(cls, rule_dict) -> int:
        keys = {'uuid', 'enable', 'type', 'priority', 'date', 'date.startDate', 'date.endDate', 'time',
        ...
        # 验证时间和日期的有效性
        time_value_dict = {value: 'TIME' for item in rule_dict['time'] for value in item.values()}
        date_value_dict = {value: 'DATE' for item in rule_dict['date'] for value in item.values()}
        if not cls.__check_values_validate({**time_value_dict, **date_value_dict}):
            # 返回参数错误
            msg = MyLog.color_red('date or time value invalid')
            MyLog.logger.error(msg)
            return g_retValue.qjBoxOpcodeInputParamErr.value
